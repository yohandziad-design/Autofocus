      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
          {formulasDisplay.map((formula, index) => (
            <div
              key={formula.key}
              className={`card-premium p-6 sm:p-8 relative ${
                formula.popular ? "md:scale-105 ring-2 ring-cyan-400/50" : ""
              }`}
            >
              {formula.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-gradient-to-r from-cyan-400 to-blue-500 text-black px-4 py-1.5 rounded-full text-xs sm:text-sm font-bold">
                    ⭐ Plus populaire
                  </span>
                </div>
              )}
              ...
              <div className="text-center mb-6 sm:mb-8">
                <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">{formula.name}</h3>
                <div className="text-3xl sm:text-4xl font-bold text-cyan-400 mb-2">{formula.price}</div>
                <p className="text-slate-400 text-sm sm:text-base">{formula.duration}</p>
              </div>

              <ul className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                {formula.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
